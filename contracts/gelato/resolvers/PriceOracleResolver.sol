...
contract PriceOracleResolver is Ownable {
    ...
    function addOracle(
        ...
        require(oracle[_oracle] == address(0), "PriceOracleResolver.addOracle: set");
    ...
    function getPrice(string memory _oracle) external view returns (uint256) {
        ...
        if (oracleAddr == address(0)) revert("PriceOracleResolver.getPrice: no oracle");
        (bool success, bytes memory returndata) = oracleAddr.staticcall(oraclePayload[_oracle]);
        if (!success) returndata.revertWithError("PriceOracleResolver.getPrice:");
